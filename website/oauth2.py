...
from .models import (OAuth2AuthorizationCode, OAuth2Client, OAuth2Token, User,
                     db)
...
class MyOpenIDCode(OpenIDCode):
    ...
    def get_jwt_config(self, grant):
        current_app.logger.debug(
            "get_jwt_config called for client %s", grant.client.client_id
        )
        return {
    ...
    def check_response_type(self, response_type):
        current_app.logger.debug(
            "OpenIDCode.check_response_type called with: %s", response_type
        )
...
def config_oauth(app):
    ...
    authorization.register_grant(
        ...
        [MyOpenIDCode(require_nonce=False), CodeChallenge(required=True)],
    )
